...
ODC_model_path = os.path.join(base_dir, 'mmselfsup2')
sys.path.append(ODC_model_path)
...
class MultiLevelConv1DFingerprinting(nn.Module):
    def __init__(self,feature_width=32,nb_classes=None):
        ...
        self._34Dense = nn.Sequential(nn.Linear(in_features=256,out_features=feature_width), nn.ReLU(True))  
...
class ODCModel(nn.Module):
    def __init__(self, input_dim=256, hidden_dim1=512, hidden_dim2=512, output_dim=2):
        ...
        self.backbone = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=hidden_dim1, kernel_size=3, stride=1, padding=1),  
            nn.ReLU(),
            nn.Conv1d(in_channels=hidden_dim1, out_channels=hidden_dim2, kernel_size=3, stride=1, padding=1),  
            nn.ReLU(),
            nn.Conv1d(in_channels=hidden_dim2, out_channels=hidden_dim1, kernel_size=3, stride=1, padding=1),  
            nn.ReLU(),
            nn.Conv1d(in_channels=hidden_dim1, out_channels=input_dim, kernel_size=3, stride=1, padding=1),  
            nn.ReLU()
        )
    ...
    def initialize_memory_bank(self, data, num_clusters):
        self.memory_bank = {
            'features': torch.zeros((len(data), 256), dtype=torch.float32),
            'labels': torch.zeros((len(data), ), dtype=torch.long),
            'centroids': torch.zeros((num_clusters, 256), dtype=torch.float32).cuda()
        }
    ...
    def _compute_centroids_ind(self, cinds):
        num = len(cinds)
        centroids = torch.zeros((num, 256), dtype=torch.float32)   
    ...
    def deal_with_small_clusters(self):
        ...
        small_clusters = np.where(histogram <20)[0].tolist()  
...
class FECOSLModel(nn.Module):
    def __init__(self, nb_classes, use_gpu=False, device = "cpu", ratio=0.75):
        ...
        self.odc_classifier = ODCModel(256, 512, 512, nb_classes - int(nb_classes * ratio))
    ...
    def forward(self, data_loader_head, data_loader_payload, data_loader_flow, flag):
        if(flag == 'energy'):
            ...
            cls_score, features = self.odc_classifier(x4)
            del x1, x2, x3
            return x4, cls_score
