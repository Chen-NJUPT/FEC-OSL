...
def build_graph(sample, all_classes, known_classes):
    graph = dgl.DGLGraph()
    ...
    with open("dataset/flow_data/Normalize/flow_value_scope_" + str(known_classes) + "_" + str(all_classes - known_classes) + ".json", 'r') as f:
        data = json.load(f)
...
def build_graphs_from_flowcontainer_json(filename, time_period, concurrent_time_threshold=concurrent_time_threshold, all_classes = 8, known_classes = 6):         
